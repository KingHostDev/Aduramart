        ...
        {products.length ? products.map((product) => (
          <div key={product.id} className="grid gap-3 border-b border-[#ece6ff] bg-white p-4 last:border-b-0 md:grid-cols-[1fr_auto]">
            ...
            <div className="flex flex-wrap items-center gap-2 md:justify-end">
              ...
              {canManageStore ? <ProductActions product={product} /> : <Link href="/vendor/dashboard/messages" className="rounded-full border border-[#fed7aa] bg-white px-3 py-1 text-xs font-extrabold text-[#B96312]">Contact admin</Link>}
            </div>
          </div>
        )) : <div className="bg-white p-6"><EmptyState title="No products submitted" copy="Approved vendors can submit products for admin review from the submit form." /></div>}
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      {product.status === "approved" ? (
        <form action={markVendorProductOutOfSale}><input type="hidden" name="productId" value={product.id} /><button className="rounded-full border border-[#ffe1bd] bg-white px-3 py-1 text-xs font-extrabold text-[#B96312]">Out of sale</button></form>
      ) : null}
...
export function SubmitListingForm({ vendor, canSubmitProducts }: { vendor: Vendor; canSubmitProducts: boolean }) {
  return (
    <form id="submit-listing" action={submitProductForReview} className="card rounded-[22px] p-6">
...
export function VendorAccountSettings({ vendor }: { vendor: Vendor }) {
  return (
    <section className="card rounded-[22px] border border-[#fecaca] p-6">
      ...
      <p className="mt-2 text-sm font-bold leading-7 text-[#6B7280]">Delete account lives here so it is not mixed with selling tasks. This action cannot be undone.</p>
